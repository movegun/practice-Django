...
def home(request):
    ...
    # 결론 : 1등테마와 1등테마의 순위가 1등인 row를 가져와라!
    context={
...
def logout(request):
    ...
    return redirect("/")
def ehdrjs(request):     
    template = loader.get_template('ehdrjs.html')
    return HttpResponse(template.render({}, request))
...
def new_keyword(request):
    template = loader.get_template('new_keyword.html')
    list_total_play = All_url.objects.values('theme').annotate(total_play=Sum('win_num')).order_by('-total_play')
    #print("list_total_play :",type(list_total_play),list_total_play)
    #[{'theme': 'pokemon', 'total_play': 52},
    # {'theme': 'celeb', 'total_play': 26},
...
def insert_new_keyword(request,selected_theme,insert_name):
    
    from insert_1_keyword import insert_one_keyword
    
    insert_one_keyword(selected_theme,insert_name)    
    
    return redirect("/home")

def admin_check(request):        
    template = loader.get_template('admin_check.html')
                
    if request.method == 'POST':
        True_or_False =False
        print(request.POST.get('admin_id_name'))
        print(request.POST.get('admin_pw_name'))
        if request.POST.get('admin_pw_name')=="apple":
            True_or_False = True            
        else:
            True_or_False = False
        context={
